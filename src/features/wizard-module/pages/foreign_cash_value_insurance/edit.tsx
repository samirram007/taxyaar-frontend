...
const ForeignCashValueInsuranceEdit = () => {
  const router = useRouter()
  const location = useLocation()
  const pathSegments = location.pathname.split('/')
  const id = pathSegments[pathSegments.length - 1]
  const { getInsuranceById, updateInsurance } = useCashValueInsurance()
  ...
  useEffect(() => {
    const insurance = getInsuranceById(parseInt(id))
    if (insurance) {
  ...
  const handleSave = () => {
    ...
    updateInsurance(parseInt(id), data)
...
export default ForeignCashValueInsuranceEdit

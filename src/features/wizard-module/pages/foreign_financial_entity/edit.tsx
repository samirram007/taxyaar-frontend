...
const ForeignFinancialEntityEdit = () => {
  const router = useRouter()
  const location = useLocation()
  const pathSegments = location.pathname.split('/')
  const id = pathSegments[pathSegments.length - 1]
  const { getEntityById, updateEntity } = useFinancialEntities()
  ...
  useEffect(() => {
    const entity = getEntityById(parseInt(id))
    if (entity) {
      setFormData(entity)
    }
  }, [id, getEntityById])
  ...
  const handleSave = () => {
    updateEntity(parseInt(id), formData)
...
export default ForeignFinancialEntityEdit

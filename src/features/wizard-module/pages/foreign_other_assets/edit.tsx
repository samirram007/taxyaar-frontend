...
const ForeignOtherAssetsEdit = () => {
  const router = useRouter()
  const location = useLocation()
  const pathSegments = location.pathname.split('/')
  const id = pathSegments[pathSegments.length - 1]
  const { getAssetById, updateAsset } = useOtherAssets()
  ...
  const handleSave = () => {
    updateAsset(parseInt(id), formData)
    router.navigate({ to: '/foreign_other_assets' })
  }
              ...
              <div className="space-y-6 px-6">
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={formData.natureOfAsset}
                    onChange={(e) =>
                      handleChange('natureOfAsset', e.target.value)
                    }
                  />
                </div>
...
export default ForeignOtherAssetsEdit

...
const ForeignOtherIncomeEdit = () => {
  const router = useRouter()
  const location = useLocation()
  const pathSegments = location.pathname.split('/')
  const id = pathSegments[pathSegments.length - 1]
  const { getIncomeById, updateIncome } = useForeignIncome()
  ...
  useEffect(() => {
    const income = getIncomeById(parseInt(id))
    if (income) {
      setFormData(income)
    }
  }, [id, getIncomeById])
  ...
  const handleSave = () => {
    updateIncome(parseInt(id), formData)
...
export default ForeignOtherIncomeEdit
